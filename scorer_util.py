...
def print_eval_results(mention_eval_out, all_attribute_combinations):
    ...
    for attr_index, attr_based_score in enumerate(attribute_based_global_scores):
        ...
        #print(attribute_header_list[attr_index + 1])
        if attribute_header_list[attr_index + 1].strip() == 'mention_type':
            returndict['mention_type'] = attr_average_scores

        if attribute_header_list[attr_index + 1].strip() == 'realis_status':
            returndict['realis_status'] = attr_average_scores
    ...
    if len(EvalState.overall_coref_scores) > 0:
        ...
        for metric, score in EvalState.overall_coref_scores.items():
            formatter = "Metric : %s\tScore\t%.2f\t%.2f\t%.2f\n"
            if metric in Config.skipped_metrics:
                formatter = "Metric : %s\tScore\t%.2f\t%.2f\t%.2f *\n"
            else:
                conll_sum += score[0]
                num_metric += 1
            mention_eval_out.write(formatter % (metric, score[0], score[1], score[2]))
            returndict['coref'].append((metric, score[0]))
        mention_eval_out.write(
            "Overall Average CoNLL score\t%.2f\n" % (conll_sum / num_metric))
        returndict['coref'].append(("avg", (conll_sum / num_metric)))
...
trigger_existence_gold = {'other': {}, 'verb': {}}  # MODIFIED
trigger_existence_sys = {'other': {}, 'verb': {}}  # MODIFIED
tp_triggers = {'other': [], 'verb': []}  # MODIFIED
other_triggers = {'other': [], 'verb': []}  #MODIFIED
def per_type_eval(system_mention_table, gold_mention_table, type_mapping):
    ...
    """
    # print type_mapping
    global trigger_existence_gold # MOFIDIED
    global trigger_existence_sys # MOFIDIED
    global tp_triggers # MOFIDIED
    global other_triggers # MOFIDIED
    for gold_index, (sys_index, score) in enumerate(type_mapping):
        ...
        if sys_index >= 0:
            ...
            tp_triggers[mention_type].append(canonicalize_string(gold_mention_table[gold_index][4])) #MODIFIED
        else: #MODIFIED
            other_triggers[mention_type].append(canonicalize_string(gold_mention_table[gold_index][4])) #MODIFIED# MODIFIED

    for gold_row in gold_mention_table:
        attributes = gold_row[1]
        mention_type = attributes[0]
        utils.put_or_increment(EvalState.per_type_num_gold, mention_type, 1)
        if mention_type == 'other' or mention_type == 'verb': # MOFIDIED
            trigger_existence_gold[mention_type][canonicalize_string(gold_row[4])] = 1 # MOFIDIED
    ...
    for sys_row in system_mention_table:
        ...
        if mention_type == 'other' or mention_type == 'verb': # MOFIDIED
            trigger_existence_sys[mention_type][canonicalize_string(sys_row[4])] = 1 # MOFIDIED
    ...
    with open('trigger_existence_gold.temp.pkl', 'wb') as f:
        pickle.dump(trigger_existence_gold, f)
    with open('tp_triggers.temp.pkl', 'wb') as f:
        pickle.dump(tp_triggers, f)
    with open('other_triggers.temp.pkl', 'wb') as f:
        pickle.dump(other_triggers, f)
...
def evaluate(token_dir, coref_out, all_attribute_combinations, token_offset_fields, token_file_ext, diff_out):
    ...
    for x, y in zip(greedy_attribute_tps, attribute_based_fps):
        attribute_based_counts.append([x, y])

    EvalState.doc_mention_scores.append((greedy_tp, fp, attribute_based_counts, num_gold_predictions, num_system_predictions, doc_id))
